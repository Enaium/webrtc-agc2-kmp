 ...
 * Ships webrtc_agc2_jni.dll as a classpath resource at
 * /cn/enaium/webrtc/agc2/native/windows-x86_64/, which NativeLoader
...
val libFile = "webrtc_agc2_jni.dll"
...
// via MSYS2). MSVC is avoided because the webrtc-agc2 C++ code relies on
...
val configureJniLibrary by tasks.registering(Exec::class) {
    group = "build"
    description = "cmake-configures webrtc_agc2_jni for $classifier."
    onlyIf { canBuildHere }
...
val buildJniLibrary by tasks.registering(Exec::class) {
    group = "build"
    description = "Builds webrtc_agc2_jni.dll for $classifier."
    onlyIf { canBuildHere }
